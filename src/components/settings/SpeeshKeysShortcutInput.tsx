...
interface SpeeshKeysShortcutInputProps {
...
export const SpeeshKeysShortcutInput: React.FC<SpeeshKeysShortcutInputProps> = ({
  ...
  const cancelRecording = useCallback(async () => {
    ...
    await commands.stopSpeeshKeysRecording().catch(console.error);
  ...
  // Set up event listener for speesh-keys events
  useEffect(() => {
    if (!isRecording) return;
    ...
    const setupListener = async () => {
      ...
      const unlisten = await listen<SpeeshKeysEvent>(
        "speesh-keys-event",
        async (event) => {
          if (cleanup) return;
    ...
    return () => {
      ...
      commands.stopSpeeshKeysRecording().catch(console.error);
    };
  }, [
    isRecording,
    shortcutId,
  ...
  const startRecording = async () => {
    ...
    try {
      await commands.startSpeeshKeysRecording(shortcutId);
      setIsRecording(true);
      setCurrentKeys("");
      currentKeysRef.current = "";
    } catch (error) {
