...
//! - `speesh_keys`: Uses the speesh-keys library for more control
//!
...
// Note: Commands are accessed via shortcut::speesh_keys:: in lib.rs
...
pub fn init_shortcuts(app: &AppHandle) {
    ...
    match user_settings.keyboard_implementation {
        ...
        KeyboardImplementation::SpeeshKeys => {
            if let Err(e) = speesh_keys::init_shortcuts(app) {
                error!("Failed to initialize speesh-keys shortcuts: {}", e);
                ...
                // Update settings to persist the fallback so we don't retry SpeeshKeys on next launch
...
pub fn register_cancel_shortcut(app: &AppHandle) {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::SpeeshKeys => speesh_keys::register_cancel_shortcut(app),
    }
}
...
pub fn unregister_cancel_shortcut(app: &AppHandle) {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::SpeeshKeys => speesh_keys::unregister_cancel_shortcut(app),
    }
}
...
pub fn register_shortcut(app: &AppHandle, binding: ShortcutBinding) -> Result<(), String> {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::SpeeshKeys => speesh_keys::register_shortcut(app, binding),
    }
}
...
pub fn unregister_shortcut(app: &AppHandle, binding: ShortcutBinding) -> Result<(), String> {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::SpeeshKeys => speesh_keys::unregister_shortcut(app, binding),
    }
}
...
pub fn change_keyboard_implementation_setting(
    ...
    // Initialize new implementation if needed (SpeeshKeys needs state)
    if new_impl == KeyboardImplementation::SpeeshKeys && initialize_speesh_keys_with_rollback(&app)? {
...
pub fn get_keyboard_implementation(app: AppHandle) -> String {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::SpeeshKeys => "speesh_keys".to_string(),
    }
}
...
fn validate_shortcut_for_implementation(
    ...
    match implementation {
        ...
        KeyboardImplementation::SpeeshKeys => speesh_keys::validate_shortcut(raw),
    }
}
...
fn parse_keyboard_implementation(s: &str) -> KeyboardImplementation {
    match s {
        ...
        "speesh_keys" => KeyboardImplementation::SpeeshKeys,
        other => {
            warn!(
...
/// Initialize SpeeshKeys if not already initialized, with rollback on failure
fn initialize_speesh_keys_with_rollback(app: &AppHandle) -> Result<bool, String> {
    if app.try_state::<speesh_keys::SpeeshKeysState>().is_some() {
        return Ok(false); // Already initialized, caller should continue
    }

    if let Err(e) = speesh_keys::init_shortcuts(app) {
        error!("Failed to initialize SpeeshKeys: {}", e);
        // Rollback to Tauri
        let mut settings = settings::get_settings(app);
        settings.keyboard_implementation = KeyboardImplementation::Tauri;
        settings::write_settings(app, settings);
        tauri_impl::init_shortcuts(app);
        return Err(format!(
            "Failed to initialize SpeeshKeys: {}. Reverted to Tauri.",
            e
        ));
    }
